...
const Navbar = () => {
    ...
    return ( <>{showNavbar && <nav className='bg-white flex justify-between items-center fixed top-2.5 sm:top-5 lg:top-8 left-1/2 -translate-x-1/2 w-[94%] sm:w-[90%] lg:w-[82%] max-w-7xl rounded-full p-1.5 px-2.5 sm:p-3 sm:px-5 lg:p-4 lg:px-6 shadow-md z-50'>
            ...
            <div className='flex gap-1 sm:gap-2 lg:gap-3 items-center shrink-0'>
                <Link href="/my-links">
                    <button className="bg-pink-300 hover:bg-pink-400 text-gray-900 font-bold px-1.5 sm:px-2.5 lg:px-4 py-1 sm:py-1.5 lg:py-2 rounded-full text-[10px] sm:text-xs md:text-sm lg:text-base whitespace-nowrap transition-colors">My Claimed Links</button>
                </Link>
                <button className="login bg-gray-400 px-1.5 sm:px-2.5 lg:px-4 py-1 sm:py-1.5 lg:py-2 rounded-lg font-bold text-[10px] sm:text-xs md:text-sm lg:text-base">Log in</button>
                <button className="signup bg-gray-900 text-white font-bold px-1.5 sm:px-2.5 lg:px-4 py-1 sm:py-1.5 lg:py-2 rounded-full text-[10px] sm:text-xs md:text-sm lg:text-base whitespace-nowrap">Sign up free</button>
            </div>
        </nav>}
        </>
